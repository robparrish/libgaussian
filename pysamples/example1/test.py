...
print '  Overlap'
S = Tensor.build(TensorType.kCore, "S", d)
ob.compute_S(S)
#S.printf()
...
print '  Kinetic'
T = Tensor.build(TensorType.kCore, "T", d)
ob.compute_T(T)
#T.printf()

print '  Potential'
V = Tensor.build(TensorType.kCore, "V", d)
ob.compute_V_nuclear(V,smol)
#V.printf()
    
print ''

H = Tensor.build(TensorType.kCore, "H", d)
H.permute(T,build_indices(["i","j"]),build_indices(["i","j"]),1.0,0.0)
H.permute(V,build_indices(["i","j"]),build_indices(["i","j"]),1.0,1.0)

F = Tensor.build(TensorType.kCore, "F", d)
Ft1 = Tensor.build(TensorType.kCore, "Ft1", d)
Ft2 = Tensor.build(TensorType.kCore, "Ft2", d)
C = Tensor.build(TensorType.kCore, "C", d)
D = Tensor.build(TensorType.kCore, "D", d)
Cocc = Tensor.build(TensorType.kCore, "Cocc", build_dimension([nbf,nocc]))
J = Tensor.build(TensorType.kCore, "D", d)
K = Tensor.build(TensorType.kCore, "K", d)
G = Tensor.build(TensorType.kCore, "G", d)
E = Tensor.build(TensorType.kCore, "E", build_dimension([]))
...
for ind in range(0,50):
    ...
    #jk.compute_JK_from_D(Dvec,TrueVec,Jvec,Kvec)    

    F.copy(H)
    F.permute(J,build_indices(["i","j"]),build_indices(["i","j"]),2.0,1.0)
    F.permute(K,build_indices(["i","j"]),build_indices(["i","j"]),-1.0,1.0)
    ...
    Ft1.contract(X,F,build_indices(["i","q"]),build_indices(["i","p"]),build_indices(["p","q"]),1.0,0.0)
    Ft2.contract(Ft1,X,build_indices(["i","j"]),build_indices(["i","q"]),build_indices(["q","j"]),1.0,0.0)
    data = Ft2.syev(EigenvalueOrder.kAscending)
    F2 = data['eigenvectors']
    C.contract(X,F2,build_indices(["p","i"]),build_indices(["p","j"]),build_indices(["i","j"]),1.0,0.0)
